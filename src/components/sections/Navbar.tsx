...
export function Navbar() {
  ...
  return (
    <>
      <nav className="nav-glass fixed left-0 right-0 top-0 z-50 flex h-16 items-center justify-between"
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button onClick={() => setOpen(o => !o)} aria-label="Toggle menu"
            ...
            <span className="block h-[2px] w-5 rounded-full transition-all duration-300"
              style={{ background: "var(--text)", transform: open ? "translateY(7px) rotate(45deg)" : "none" }} />
            <span className="block h-[2px] w-5 rounded-full transition-all duration-300"
      ...
      <div className="fixed inset-x-0 top-16 z-40 md:hidden"
        style={{ pointerEvents: open ? "all" : "none", opacity: open ? 1 : 0, transform: open ? "translateY(0)" : "translateY(-8px)", transition: "opacity 0.2s ease, transform 0.2s ease", background: "var(--card-bg)", borderBottom: "1px solid var(--card-border)", boxShadow: "0 8px 32px rgba(0,0,0,0.15)" }}>
        <div className="flex flex-col gap-1 px-4 py-4">
          {NAV_LINKS.map(({ label, href }) => (
            <Link key={href} href={href}
              className="flex items-center gap-3 rounded-xl px-4 py-3 font-mono text-[14px] transition-colors"
              style={{ color: path === href ? "var(--brand)" : "var(--text)", background: path === href ? "var(--chip-bg)" : "transparent" }}>
              <span className="h-1.5 w-1.5 rounded-full" style={{ background: path === href ? "var(--brand)" : "var(--border)" }} />
              {label}
            </Link>
          ))}
          <a href={`mailto:${siteConfig.email}`}
      ...
      {open && <div className="fixed inset-0 z-30 md:hidden" onClick={() => setOpen(false)} style={{ background: "rgba(0,0,0,0.2)" }} />}
    </>
  );
}
