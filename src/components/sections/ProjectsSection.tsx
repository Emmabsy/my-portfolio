...
const ACCENT: Record<string, { panel: string; dot: string; tagBg: string; tagBorder: string; glyph: string; year: string }> = {
  ...
  "prompt-bible":        { panel: "linear-gradient(140deg,#1a0a2e,#2d1a5c,#0f0720)", dot: "#c084fc", tagBg: "rgba(192,132,252,0.15)", tagBorder: "rgba(192,132,252,0.3)", glyph: "AI",   year: "2025" },
  "launchkit":           { panel: "linear-gradient(140deg,#0a1628,#1a2d50,#080e1a)", dot: "#38bdf8", tagBg: "rgba(56,189,248,0.15)",  tagBorder: "rgba(56,189,248,0.3)",  glyph: "KIT",  year: "2025" },
  ...
  "welaspace":           { panel: "linear-gradient(140deg,#0a1a30,#162a4a,#060d18)", dot: "#34d399", tagBg: "rgba(52,211,153,0.15)",  tagBorder: "rgba(52,211,153,0.3)",  glyph: "WS",   year: "2024" },
  "devflow-pro":         { panel: "linear-gradient(140deg,#0f0a20,#231840,#0a0615)", dot: "#f472b6", tagBg: "rgba(244,114,182,0.15)", tagBorder: "rgba(244,114,182,0.3)", glyph: "DEV",  year: "2024" },
  "ghostorganize-pro":   { panel: "linear-gradient(140deg,#0a1a10,#183028,#050e08)", dot: "#4ade80", tagBg: "rgba(74,222,128,0.15)",  tagBorder: "rgba(74,222,128,0.3)",  glyph: "GO",   year: "2024" },
  "edu-platform":        { panel: "linear-gradient(140deg,#1a1000,#2d1f00,#120c00)", dot: "#fbbf24", tagBg: "rgba(251,191,36,0.15)",  tagBorder: "rgba(251,191,36,0.3)",  glyph: "EDU",  year: "2025" },
  ...
  "stockcard-twin":      { panel: "linear-gradient(140deg,#001a1a,#003434,#000d0d)", dot: "#22d3ee", tagBg: "rgba(34,211,238,0.15)",  tagBorder: "rgba(34,211,238,0.3)",  glyph: "DT",   year: "2024" },
  "nairobi-rush":        { panel: "linear-gradient(140deg,#1a0a00,#3a1a00,#0d0500)", dot: "#f97316", tagBg: "rgba(249,115,22,0.15)",  tagBorder: "rgba(249,115,22,0.3)",  glyph: "GAME", year: "2023" },
  "grid-defender":       { panel: "linear-gradient(140deg,#0a1a0a,#1a3a1a,#050d05)", dot: "#86efac", tagBg: "rgba(134,239,172,0.15)", tagBorder: "rgba(134,239,172,0.3)", glyph: "DEF",  year: "2023" },
  "void-drift":          { panel: "linear-gradient(140deg,#050510,#0f0f2e,#020208)", dot: "#818cf8", tagBg: "rgba(129,140,248,0.15)", tagBorder: "rgba(129,140,248,0.3)", glyph: "VOID", year: "2022" },
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      cardRefs.current.forEach((card, i) => {
        ...
        const s0 = i / exitCount, s1 = (i + 1) / exitCount;
        const t  = Math.max(0, Math.min(1, (progress - s0) / (s1 - s0)));
        const e  = t < 0.5 ? 4*t*t*t : 1 - Math.pow(-2*t+2,3)/2;
        card.style.transform  = `translateY(${-(e * 110)}%)`;
    ...
    <section id="work">
      <div className="px-[clamp(16px,5vw,80px)] pb-10 pt-16 text-center sm:pb-12 sm:pt-20">
        ...
        <p className="mx-auto mt-3 max-w-xs text-[13px] leading-relaxed" style={{ color: "var(--text-muted)" }}>
          Scroll to flip through {N} featured projects.
        </p>
      </div>
          ...
          <div style={{ width: "100%", maxWidth: 1040, padding: "0 clamp(12px,3vw,48px)" }}>
                ...
                return (
                  <div key={project.id} ref={el => { cardRefs.current[i] = el; }} style={{ position: "absolute", inset: 0 }}>
                    <div className="flex h-full w-full overflow-hidden rounded-[22px]"
                      ...
                      <div className="flex flex-1 flex-col p-5 sm:p-7 md:p-9">
                        <div className="mb-3 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            {project.wip && (
                              <span className="rounded-full px-2.5 py-0.5 font-mono text-[9px] uppercase tracking-[2px]"
                        ...
                        <div className="mt-auto pt-4 flex gap-2">
                          ...
                          {project.hasLiveDemo && project.liveUrl && (
                            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-1.5 rounded-xl px-3.5 py-2 font-mono text-[10px] text-white sm:gap-2 sm:px-4 sm:text-[11px]"
                      ...
                      {/* Right visual panel */}
                      <div className="relative hidden w-[30%] shrink-0 overflow-hidden md:block" style={{ background: acc.panel }}>
                        {project.image ? (
                          <img src={project.image} alt={project.title} className="absolute inset-0 h-full w-full object-cover opacity-40" />
                        ) : null}
                        <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(rgba(255,255,255,0.09) 1px, transparent 1px)", backgroundSize: "18px 18px" }} />
            ...
            <div className="mt-3 flex items-center justify-between px-1">
              <div className="flex gap-1.5">
                {projects.map((_, i) => <div key={i} className="rounded-full" style={{ width: 16, height: 3, background: "var(--chip-border)" }} />)}
              </div>
              <span className="hidden font-mono text-[10px] sm:block" style={{ color: "var(--text-muted)" }}>scroll to explore</span>
            </div>
          </div>
        </div>
      </div>
